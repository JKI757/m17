...
func generateStreamBits(sd StreamDatagram) ([]Bit, error) {
	...
	b, err := ConvolutionalEncodeStream(lichBits, sd.FrameNumber, sd.Payload)
	if err != nil {
...
// ProcessSymbolStream decodes a stream of filtered radio symbols.
func ProcessSymbolStream(rxSymbols <-chan float32, frameSink func(typ uint16, softBits []SoftBit), sps int) {
	processSymbolStream(rxSymbols, frameSink, sps)
}

// GenerateLSFBits encodes an LSF for a radio frame.
func GenerateLSFBits(lsf LSF) ([]Bit, error) { return generateLSFBits(lsf) }

// GenerateLSFSymbols encodes an LSF as radio symbols.
func GenerateLSFSymbols(lsf *LSF) ([]Symbol, error) { return generateLSFSymbols(lsf) }

// GenerateStreamBits encodes a stream datagram for a radio frame.
func GenerateStreamBits(sd StreamDatagram) ([]Bit, error) { return generateStreamBits(sd) }

// GenerateStreamSymbols encodes a stream datagram as radio symbols.
func GenerateStreamSymbols(sd StreamDatagram) ([]Symbol, error) { return generateStreamSymbols(sd) }
...
// ExtractPayload returns the symbols and soft bits after a synchronization pattern.
func ExtractPayload(dist float32, typ uint16, symbols []Symbol, sps int) ([]Symbol, []SoftBit, float32) {
	return extractPayload(dist, typ, symbols, sps)
}

// PackBits converts bits to bytes in network bit order.
func PackBits(in []Bit) []byte { return packBits(in) }

// UnpackBits converts bytes to bits in network bit order.
func UnpackBits(in []byte) []Bit { return unpackBits(in) }
